...
uint8_t fat_table_buffer[FAT_SECTOR_SIZE*50]; //Just the first 50 sectors in the FAT table
...
uint32_t fat_init( void ){
    ...
    //read fat fat_table (just the first 50 sectors)
...
void fat_file_read( FATFile* file,  uint8_t* buf ){
    ...
    }while( cluster != FAT_END_OF_CHAIN_MARKER );
...
uint32_t fat_read_files_in_dir( FATDirectory* dir, uint8_t* path ){
    ...
    //
    uint32_t dir_table_sector = boot_sector.data_region;
    ...
    for(uint32_t base=0; base<FAT_MAX_DIR_ENTRIES*FAT_DIR_ENTRY_SIZE; base+=FAT_DIR_ENTRY_SIZE){
